...
export default class Constructor {
  ...
  async init(gl: WebGL2RenderingContext | null) {
    ...
    mat4.translate(transform2, transform2, vec3.fromValues(-0.5, 0.5, 0.0));
    const scaleAmount = Math.sin(performance.now() / 1000.0);
    mat4.scale(
      transform2,
      transform2,
      vec3.fromValues(scaleAmount, scaleAmount, scaleAmount)
    );
