...
export default class Constructor {
  ...
  yaw: number = -90.0; // yaw is initialized to -90.0 degrees since a yaw of 0.0 results in a direction vector pointing to the right so we initially rotate a bit to the left.
  pitch: number = 0.0;
  lastX: number = 800.0 / 2.0;
  lastY: number = 600.0 / 2.0;
  fov: number = 45.0;
  ...
  updateCameraPosByWheel(event: WheelEvent): void {
    // event.deltaY 表示垂直滚轮方向
    // 通常：向上滚 event.deltaY < 0，向下滚 event.deltaY > 0

    if (event.deltaY < 0) {
      this.fov -= 1.0; // 调整步长，可以改
    } else {
      this.fov += 1.0;
    }
