...
export default class Constructor {
  ...
  async init(gl: WebGL2RenderingContext | null) {
    ...
    // light properties
    const lightColor = [
      Math.sin(currentFrame * 2.0), // lightColor.x
      Math.sin(currentFrame * 0.7), // lightColor.y
      Math.sin(currentFrame * 1.3), // lightColor.z
    ];
    // 计算漫反射颜色（降低影响）
    const diffuseColor = [
      lightColor[0] * 0.5, // diffuseColor = lightColor * 0.5f
      lightColor[1] * 0.5,
      lightColor[2] * 0.5,
    ];
    // 计算环境光颜色（低影响）
    const ambientColor = [
      diffuseColor[0] * 0.2, // ambientColor = diffuseColor * 0.2f
      diffuseColor[1] * 0.2,
      diffuseColor[2] * 0.2,
    ];
    // 镜面反射光颜色（固定为白色）
    ...
    this.lightingShader.setVec3("light.specular", specularColor);

    // material properties
    // 材质属性设置（对应material properties）
    const materialAmbient = [1.0, 0.5, 0.31]; // material.ambient
    const materialDiffuse = [1.0, 0.5, 0.31]; // material.diffuse
    const materialSpecular = [0.5, 0.5, 0.5]; // material.specular (镜面反射效果不完全)
    const materialShininess = 32.0; // material.shininess
    this.lightingShader.setVec3("material.ambient", materialAmbient);
    this.lightingShader.setVec3("material.diffuse", materialDiffuse);
    this.lightingShader.setVec3("material.specular", materialSpecular);
