...
export default class Constructor {
  ...
  async init(gl: WebGL2RenderingContext | null) {
    ...
    this.lightingShader.setVec3("light.ambient", whiteColor);
    this.lightingShader.setVec3("light.diffuse", whiteColor);
    this.lightingShader.setVec3("light.specular", whiteColor);

    // material properties
    // 材质属性设置（对应material properties）
    const materialAmbient = [0.0, 0.1, 0.06]; // material.ambient
    const materialDiffuse = [0.0, 0.50980392, 0.50980392]; // material.diffuse
    const materialSpecular = [0.50196078, 0.50196078, 0.50196078]; // material.specular (镜面反射效果不完全)
